...
class RegSeqDB:
	...
	######################################
	# Database Methods
	def self.connect(self, host, port, database, username, password):
	...
	def __query(self, query, inputs):
		...
		return (
			...
			query_output["colnames"],
			query_output["rowcount"]
		)
	...
	def get_promoter_expr(self, promoter, condition):
        ...
        # Submit Query
        results, colnames, rowcount = self.__query(query, inputs)
		return

	# ######################################
	# # Query Wrappers
	# def get_promoter_expr_and_binding(self, promoter, condition, filter):
	# 	"""Get Promoter Expression 
